  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Restrict run instance to specific vpc and subnets.
      {
        ...
        Effect : "Allow",
        Action : "ec2:RunInstances",
        Resource : formatlist("arn:${local.partition}:ec2:${var.aws_cluster_region}:${local.account_id}:subnet/%s", var.subnets)
        Condition : {
          StringEquals : {
      ...
      # Restrict scaling down autoscaling groups to specific cluster.
      {
        ...
        Condition : {
          StringEquals : {
            "autoscaling:ResourceTag/kubernetes.io/cluster/${var.aws_cluster_name}" : [
              "owned",
              "shared"
            ]
          }
